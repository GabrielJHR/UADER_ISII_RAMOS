        ...
        if(userText):
            try:
                ...
                if("--convers" in buffer or "--convers" in userText):
                    # Si se indica que la conversación está en curso, se agrega la entrada del usuario
                    # al buffer y se muestra la respuesta de ChatGPT
                    buffer+=f"You: {userText}\n"
                else:
                    # Si no se indica que la conversación está en curso, se sobrescribe el contenido del buffer
